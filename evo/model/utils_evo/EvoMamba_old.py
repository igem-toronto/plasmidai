...
class EvoMamba(nn.Module):
    def __init__(self, config):
        ...
        self.mamba = vm.videomamba_tiny(output_evo_num_channels=self.config['evo_output_features'], embed_dim=self.config['embed_channels'])
    ...
    def forward(self, x):
        ...
        if self.config['full_debug']:
            ...
            print('Here is the input format', x.shape) # Here is the input format torch.Size([8, 512, 4096])
...
class PatchEmbed(nn.Module):
    def __init__(self, config):
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, self.evo_input_embed_dim, self.evo_input_length))
    ...
    def forward(self, x):
        x = self.proj(x)
        x = rearrange(x, 'b (c d) -> b c d', c=self.evo_input_embed_dim, d=self.evo_input_length)
