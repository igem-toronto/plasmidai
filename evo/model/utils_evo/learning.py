...
def configure_optim_scheduler(config, model):
    ...
    if config['optimizer'] == 'adam':
        optimizer = optim.Adam(filter(
            lambda p: p.requires_grad, model.parameters()), lr=config['learning_rate'])
    ...
    if config['scheduler_fct'] == 'RRLP':
        scheduler = optim.lr_scheduler.ReduceLROnPlateau(
            optimizer=optimizer, factor=0.5)  # half the learning rate each time
    ...
    if config['scheduler_fct'] == 'cosine':
        ...
        scheduler = optim.lr_scheduler.CosineAnnealingWarmRestarts(
            optimizer=optimizer, T_0=T_0, T_mult=T_mult, eta_min=eta_min, last_epoch=last_epoch)
...
def run_step(model, epoch, first_epoch, config, data, loss_fn, batch_idx, split):
    ...
    if first_epoch and batch_idx == 0:
        print(
            f'[INFO] Memory usage per step: {torch.cuda.memory_reserved()/1e6} (MB)')
...
def load_weights(model, checkpoint_path):
    ...
    if os.path.exists(checkpoint_path):
        ...
        merged_state_dict = {k: v for k, v in state_dict.items() if k in model_state_dict and k in state_dict and model_state_dict[k].shape == state_dict[k].shape}

        if merged_state_dict != state_dict:
            print("[WARNING] The weights of you model had different shapes from the weights that you loaded")
            print("[WARNING] Here are the differing parameters: ", model_state_dict.keys() ^ state_dict.keys())
