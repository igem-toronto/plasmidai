...
class EvoLoss(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.crossentropy_loss = F.cross_entropy #(logits, dnas[..., 1:], reduction="none")  # (... L)
    ...
    def forward(self, expected_output, predicted_output):
        ...
        if 'perplexity' in self.config['loss_type']:
            computed_loss += self.perplexity_loss(expected_output, predicted_output) * self.config['loss_type']['perplexity']

        if 'crossentropy' in self.config['loss_type']:
            computed_loss += self.crossentropy_loss(expected_output, predicted_output, reduction="none") * self.config['loss_type']['crossentropy']
