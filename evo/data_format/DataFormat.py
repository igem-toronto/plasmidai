...
class PlasmidDataset(Dataset):
    def __init__(self, config, train_set, real_job, tokenizer=None):
        ...
        self.train_name, self.test_name = self.read_splits(self.splits_path, self.real_job)
        self.sequences = self.read_sequences(self.data_path, self.length_seq, self.train_set)
        
        if self.config['use_custom_tokens']:
            self.tokenizer = define_tokenizer(self.config['root'], os.path.join(os.getcwd(), 'data_format/tokenizer', self.config['tokenizer']))
        else:
        ...
        print(f'Train is set to {self.train_set}, and the number of sequences is {len(self.treated_sequences)}')
    ...
    def __getitem__(self, idx):
        ...
        return sequence, mask#, output_label # mask says where the input was truncated
    ...
    def masking_sequence(self, tokenized_sequence):
        ...
        masked_sequence = tokenized_sequence[:mask_position] # + [9] #+ tokenized_sequence[mask_position + 1:]
    ...
    def read_sequences(self, data_path, length_seq, train_set):
        ...
        records = [r for r in SeqIO.parse(data_path, "fasta") if len(r.seq) < length_seq]
        ...
        for record in records:
            record_data = [record.id, record.name, record.description, str(record.seq), record.annotations, record.letter_annotations]
